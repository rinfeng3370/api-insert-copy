...
class queueTest extends Command
{
    ...
    {
        if(!$this->date && !$this->startNum && !$this->endNum){    //避免需要重複輸入
            $this->date = $this->ask('請輸入要查詢的日期("YYYY-MM-DD")');
            
            $this->verifyDate($this->date);

            $this->startNum = $this->ask('請輸入起始筆數("0或萬為單位，最多90000")');

            $this->endNum = $this->ask('請輸入最終筆數("以萬為單位，最多100000")');

            $this->verifyNum($this->startNum,$this->endNum);
        ...
        $url = "http://train.rd6/?start={$this->date}T{$this->hour}:{$this->min}:00&end={$this->date}T{$this->hour}:{$this->min}:59&from={$this->startNum}";
        echo $url. "\n";
        // $curl->get($url);
        ...
        if($this->startNum == ($this->endNum-10000)){
            die();
        }
    ...
    protected function verifyDate($date)  //驗證日期
    {
    ...
    protected function verifyNum($startNum,$endNum){
        ...
        if(!preg_match('/^0|[1-9]0000$/',$startNum)){     
            $this->error('請輸入正確起始筆數!');
            die();
        }

        if(!preg_match('/^[1-9]0000|100000$/',$endNum)){     
    ...
    protected function verifyTime($time){
